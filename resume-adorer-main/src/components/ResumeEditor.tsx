...
import { PaginationNext } from "./ui/pagination";
...
export const ResumeEditor: React.FC<ResumeEditorProps> = ({
  ...
  const updateHighlight = (expInd, index: number, value: string) => {
    if (!Array.isArray(data["experience"])) return;

    data.experience[expInd].responsibilities = data.experience[
      expInd
    ].responsibilities.map((val, _i) => (index === _i ? value : val));
    setData({
      ...data,
      experience: [...data.experience],
    });
  ...
  const removeHighlight = (expInd, index: number) => {
    if (!Array.isArray(data["experience"])) return;
    data.experience[expInd].responsibilities = data.experience[
      expInd
    ].responsibilities.filter((val, _i) => index != _i);
    setData({
      ...data,
      experience: [...data.experience],
    });
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {data.gaps && data.gaps?.length > 0 && (
        <Card
          className={cn(
            "p-6 transition-all duration-300 bg-red-100",
        ...
        >
          <div className="items-center">
            <h2
              className={cn("text-xl font-semibold mb-4 ", getSectionStyles())}
              style={{ display: "inline-block" }}
            >
              Gaps between JD and Resume
            </h2>
